...
impl Scenario for BasicScenario {
	async fn run(self: Box<Self>) -> Result<(), anyhow::Error> {
		...
		tracing::info!("Do transfer");

		// Have Alice send Bob some coins.
		let txn_hash = coin_client.transfer(&mut alice, bob.address(), 1_000, None).await.unwrap();
		tracing::info!("send done");

		rest_client.wait_for_transaction(&txn_hash).await.unwrap();
		tracing::info!("wait done");
